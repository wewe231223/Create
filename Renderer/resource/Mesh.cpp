...
Mesh::Mesh(ComPtr<ID3D12Device>& device, D3D12_PRIMITIVE_TOPOLOGY topology, UINT vertexCount)
	: mTopology(topology), mVertexCount(vertexCount)
{
	D3D12_RESOURCE_DESC desc{};
...
void Mesh::RenderIndexed(ComPtr<ID3D12GraphicsCommandList>& commandList)
{
...
void Mesh::RenderNonIndexed(ComPtr<ID3D12GraphicsCommandList>& commandList)
{
	commandList->SetGraphicsRootShaderResourceView(GRP_MeshConstants, mMeshContexts[mMemoryIndex].mBuffer->GetGPUVirtualAddress());
	commandList->IASetPrimitiveTopology(mTopology);
